...
const FlashLoan = require('./abis/index').flashloanv2;
const BlockSubscriber = require('./src/block_subscriber');
const Prices = require('./src/prices');

let FLASHLOAN_CONTRACT = process.env.CONTRACT;
...
const prices = {};
const flashswap = new web3.eth.Contract(FlashLoan, FLASHLOAN_CONTRACT);
        ...
        pairs.forEach((pair) => {
            calls.push(async () => {
                const check = await flashswap.methods.check(
                    pair.tokenBorrow,                           // _tokenBorrow
                    new BigNumber(pair.amountTokenPay * 1e18),  // _amountTokenPay
                    pair.tokenPay,                              // _tokenPay
                    pair.sourceRouter,                          // _sourceRouter
                    pair.targetRouter                           // _targetRouter
                ).call();
                ...
                if (profit > 0) {
                    ...
                    if (profitMinusFeeInUsd > 0.6) {
                        ...
                        try {
                            let asass = await transactionSender.sendTransaction(txData);
                        } catch (e) {
